    ...
    try:
        ...
        instruction_msg = HumanMessage(
            content=(
                ...
                f"5. AUTOMATED DEEP INTERNET SEARCH & DETAILED CREW BRIEFING WITH VENUE PHOTOS: Immediately after updating the crew, dig deep into internet research using `search_internet_tool` (running multiple context-rich queries for venue location/landmarks, reviews, technical/power specs, parking/access, website, social handles, pricing/rates, history, policies, surrounding area — e.g. '[Venue Name] Church/Hotel Harare Zimbabwe address location contact details photos' instead of generic single-word queries like 'ufic'), `search_images_tool` (to search for photos/images of the venue), and `scrape_website_tool` to read webpage content. Update any missing venue fields in `venue_venue` using `run_sql_query_tool`. Then, send a THOROUGH, RICH, AND DETAILED WhatsApp briefing message to the assigned crew ({crew_info_str}) — NOT a short summary! Structure the message clearly into sections: Location & Landmarks, Website & Social Links, Technical & Setup Specs, Venue Background & Prep Notes, AND AN ADDITIONAL SECTION FOR ANY OTHER ONLINE FINDINGS & EXTRA DETAILS. Also, send key venue photo(s) to the crew using `send_whatsapp_message_tool` with `media_url` pointing to direct Image URL(s) found via `search_images_tool` so they have clear visual context of the venue.\n"
            ...
            SystemMessage(
                content=(
                    ...
                    + (
                        ...
                        f"IMPORTANT CREW UPDATE & DEEP INTERNET SEARCH MANDATE: When you finish processing client answers or when data collection completes, you MUST use `send_whatsapp_message_tool` to update the assigned crew with ALL the info and venue details gathered so far. IMMEDIATELY AFTER THAT, dig deep into internet research using `search_internet_tool`, `search_images_tool` (to find photos of the venue), and `scrape_website_tool`. Update any missing fields in `venue_venue`, send a THOROUGH, RICH, AND DETAILED WhatsApp briefing message to the crew, AND ALSO send separate follow-up WhatsApp message(s) containing direct venue picture(s) using `send_whatsapp_message_tool` with `media_url` so the crew receives visual photos of the venue!{venue_ctx_str}"
                        if not is_crew else ""
                    )
                )
            )
