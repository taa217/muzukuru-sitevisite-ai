    ...
    try:
        ...
        instruction_msg = HumanMessage(
            content=(
                ...
                f"5. AUTOMATED DEEP INTERNET SEARCH & DETAILED CREW BRIEFING: Immediately after updating the crew, dig deep into internet research using `search_internet_tool` (running multiple queries for venue location/landmarks, reviews, technical/power specs, parking/access, website, social handles, pricing/rates, history, policies, surrounding area) and `scrape_website_tool` to read webpage content. Update any missing venue fields in `venue_venue` using `run_sql_query_tool`. Then, send a THOROUGH, RICH, AND DETAILED WhatsApp briefing message to the assigned crew ({crew_info_str}) — NOT a short summary! Structure the message clearly into sections: Location & Landmarks, Website & Social Links, Technical & Setup Specs, Venue Background & Prep Notes, AND AN ADDITIONAL SECTION FOR ANY OTHER ONLINE FINDINGS & EXTRA DETAILS. You are NOT limited to preset categories — include ANY and ALL other useful information, facts, background details, pricing, policies, contact info, or surrounding amenities found online so the crew receives 100% of the discovered data.\n"
            ...
            SystemMessage(
                content=(
                    ...
                    + (
                        ...
                        f"IMPORTANT CREW UPDATE & DEEP INTERNET SEARCH MANDATE: When you finish processing client answers or when data collection completes, you MUST use `send_whatsapp_message_tool` to update the assigned crew with ALL the info and venue details gathered so far. IMMEDIATELY AFTER THAT, dig deep into internet research using `search_internet_tool` (multiple queries for landmarks, reviews, tech/power specs, parking, website, social handles, pricing, history, policies) and `scrape_website_tool`. Update any missing fields in `venue_venue`, and send a THOROUGH, RICH, AND DETAILED WhatsApp briefing message to the crew (NOT a short 1-liner summary!). You are NOT limited to preset categories: include ANY and ALL other useful information, facts, background details, pricing, policies, contact info, or surrounding amenities found online so the crew receives 100% of the discovered data.{venue_ctx_str}"
                        if not is_crew else ""
                    )
                )
            )
