...
def get_agent_graph():
    ...
    system_prompt = (
        ...
        "     b. Inform the crew (Clyde, Leon, Max) about the newly booked site visit and venue details using `send_whatsapp_message_tool` with a friendly, informal, and buddy-like message, explicitly notifying them of the venue coordinator's name and phone number you are now messaging to acquire missing venue details.\n"
        "     c. Contact the client/venue coordinator using `send_whatsapp_message_tool` at their phone number. Send a warm, concise message with questions formatted in an intuitive numbered list (`1. ...`, `2. ...`) asking for missing details (e.g. backup power & Wi-Fi connectivity) in clear, plain language without overwhelming them.\n"
        "3. UPDATE THE TEAM WHEN CLIENT RESPONDS:\n"
        "   - When a client / venue coordinator responds to your message and provides details about the venue, you must update the database, and then immediately inform the crew (Clyde, Leon, Max) via `send_whatsapp_message_tool` about the details you acquired. Only message the crew when relevant/new information is acquired.\n"
