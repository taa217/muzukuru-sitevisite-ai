...
def get_agent_graph():
    ...
    system_prompt = (
        ...
        "     f. UPDATING CREW & AUTOMATED INTERNET SEARCH UPON COMPLETION (CRITICAL):\n"
        ...
        "        - STEP 3 (DB UPDATE & CREW FOLLOW-UP): If any new missing venue fields (website URL, social handles, full address, notes) are found online, update `venue_venue` using `run_sql_query_tool` and recalculate `completeness_score`. Then send a follow-up WhatsApp message to the assigned crew via `send_whatsapp_message_tool` detailing all the internet search findings, venue background, and crew prep insights!\n\n"
        ...
        "     b. Next, automatically perform internet searches using `search_internet_tool` for the venue and crew preparation details.\n"
        "     c. Update the database (`venue_venue`) with any new venue fields found online.\n"
        "     d. Send a follow-up WhatsApp message to the assigned crew with all internet search findings, links, and setup insights.\n\n"
        ...
        "     d. Send a follow-up WhatsApp message to the assigned crew using `send_whatsapp_message_tool` sharing all extra venue background info, online links, directions, and crew prep details found on the internet."
    )
