...
def get_agent_graph():
    ...
    system_prompt = (
        ...
        "8. SEARCHING TEXT AND IMAGES ON THE INTERNET (CONTEXT-RICH DEEP RESEARCH MANDATE):\n"
        "   - ABSOLUTE MANDATE - CONTEXTUAL SEARCH QUERIES ONLY: Never execute vague, generic, or 1-word search queries (e.g. NEVER search 'ufic', 'rainbow', or 'hicc').\n"
        "   - ALWAYS construct rich, specific search queries that combine:\n"
        "     1) Entity Full Name & Category/Type (e.g., 'UFIC Church', 'Rainbow Towers Hotel', 'Meikles Hotel', 'Wild Geese Lodge wedding venue')\n"
        "     2) Geographic Location & City (e.g., 'Harare', 'Chitungwiza', 'Bulawayo', 'Zimbabwe')\n"
        "     3) Specific Target Details (e.g., 'address location contact details photos generator PA system wifi setup guidelines')\n"
        "   - EXAMPLES OF PROPER CONTEXTUAL SEARCH QUERIES:\n"
        "     * Bad: 'ufic' -> GOOD: 'UFIC Church Harare Zimbabwe address location contact details photos'\n"
        "     * Bad: 'rainbow' -> GOOD: 'Rainbow Towers Hotel Harare Zimbabwe venue conference hall layout photos'\n"
        "     * Bad: 'hicc' -> GOOD: 'HICC Harare International Conference Centre Zimbabwe setup photos capacity'\n"
        "   - Use `search_internet_tool`, `search_images_tool`, and `scrape_website_tool` to research venues thoroughly. Dig deep by running multiple search queries.\n"
        ...
        "     a. Execute `search_internet_tool` with multiple deep, context-rich queries: e.g., '[Venue Name] [Category/Church/Hotel] Harare Zimbabwe address directions landmarks', '[Venue Name] website social media reviews', and '[Venue Name] setup guidelines parking power backup'.\n"
        "     b. Execute `search_images_tool` (e.g., '[Venue Name] [Category] photo Harare Zimbabwe', '[Venue Name] stage entrance setup layout') to find clear venue photos.\n"
