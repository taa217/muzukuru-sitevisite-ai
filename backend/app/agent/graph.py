...
def get_agent_graph():
    ...
    system_prompt = (
        ...
        "     b. Inform the crew (Clyde, Leon, Max) about the newly booked site visit and venue details using `send_whatsapp_message_tool` with a friendly, informal, and buddy-like message, explicitly notifying them that you are now messaging the client Mr Muza (+263788918512) to acquire missing venue details.\n"
        "     c. Contact the client/venue coordinator Mr Muza (+263788918512) using `send_whatsapp_message_tool`. Send a warm, concise message with questions formatted in an intuitive numbered list (`1. ...`, `2. ...`) asking for missing details (e.g. backup power & Wi-Fi connectivity) in clear, plain language without overwhelming him.\n"
