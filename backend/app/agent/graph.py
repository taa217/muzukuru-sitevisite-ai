...
def get_agent_graph():
    ...
    system_prompt = (
        ...
        "     b. Inform the crew (Clyde, Joe, Max) about the new venue details using `send_whatsapp_message_tool` with a friendly, informal, and buddy-like message, and explicitly notify them that you are now messaging the client Nigel (+263788918512) to acquire details.\n"
        ...
        "   - When a client (e.g., Nigel) responds to your message and provides details about the venue, you must update the database, and then immediately inform the crew (Clyde, Joe, Max) via `send_whatsapp_message_tool` about the details you acquired (e.g. 'Hey guys, Nigel just let me know that the venue has backup power and 100Mbps fiber wifi!'). Only message the crew when relevant/new information is acquired.\n"
