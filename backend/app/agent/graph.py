...
def get_agent_graph():
    ...
    system_prompt = (
        ...
        "        - STEP 2 (AUTOMATED INTERNET SEARCH): Immediately after updating the crew with client details, start searching the internet using `search_internet_tool` (and `scrape_website_tool` if helpful) to look up external information about that venue (such as official website, social media pages, exact address/map/directions, reviews, power/tech specs, photos, surrounding area) AND gather details relevant to the crew for event setup at that venue.\n"
        "        - STEP 3 (DB UPDATE & CREW FOLLOW-UP): If any new missing venue fields (website URL, social handles, full address, notes) are found online, update `venue_venue` using `run_sql_query_tool` and recalculate `completeness_score`. Then send a follow-up WhatsApp message to the assigned crew via `send_whatsapp_message_tool` detailing ALL the internet search findings, venue background, landmarks, directions, and crew prep insights!\n\n"
        ...
        "     b. Next, automatically perform internet searches using `search_internet_tool` for ANY venue, location, setup, and crew preparation details (not just trying to find a website URL, but ANY useful venue info online).\n"
        "     c. Update the database (`venue_venue`) with any new venue fields found online.\n"
        "     d. Send a follow-up WhatsApp message to the assigned crew with ALL internet search findings, background details, directions, links, and setup insights.\n\n"
        ...
        "   - Use `search_internet_tool` and `scrape_website_tool` to research venues, find external details (location, website, social handles, reviews, technical details, surrounding area), and gather crew preparation insights.\n"
        "   - When searching the internet, do NOT restrict yourself to finding just a website URL. Search for ANY useful details (address, directions, parking, operating hours, reviews, surrounding landmarks, power/audio info, social links, website).\n"
        "   - When client data collection finishes, AUTOMATICALLY run internet searches on the venue and crew setup details to enrich venue information and brief the team.\n\n"
        "9. AUTOMATED POST-INTERVIEW INTERNET SEARCH & CREW FOLLOW-UP WORKFLOW (CRITICAL):\n"
        "   - When client communication for data gathering completes and you have sent the initial crew briefing:\n"
        "     a. Execute `search_internet_tool` with queries like '[Venue Name] [City] address website reviews details' and '[Venue Name] setup guidelines parking'.\n"
        "     b. Use `scrape_website_tool` if a specific venue page or website URL is found to extract relevant specs or guidelines.\n"
        "     c. Update `venue_venue` with any newly discovered fields (e.g. `website`, `facebook`, `instagram`, `address_one`, `notes`) and update `completeness_score`.\n"
        "     d. Send a follow-up WhatsApp message to the assigned crew using `send_whatsapp_message_tool` sharing ALL extra venue background info, online links, directions, operating details, and crew prep insights found on the internet."
    )
