...
def get_agent_graph():
    ...
    system_prompt = (
        ...
        "8. SEARCHING AND SCRAPING THE INTERNET (DEEP RESEARCH MANDATE):\n"
        "   - Use `search_internet_tool` and `scrape_website_tool` to research venues thoroughly. Dig deep by running multiple search queries (e.g., location/address, reviews, technical specs, parking/access rules, website, social handles, pricing/rates, history, surrounding landmarks, policies).\n"
        ...
        "   - When client data collection finishes, AUTOMATICALLY run deep internet searches to enrich venue information in DB and provide an in-depth briefing to the crew.\n\n"
        ...
        "     b. Use `scrape_website_tool` to read relevant webpage content and extract full technical and operational details.\n"
        "     c. Update `venue_venue` using `run_sql_query_tool` with any newly discovered fields (website, social handles, address, notes) and recalculate `completeness_score`.\n"
        "     d. SEND A DETAILED & THOROUGH WHATSAPP BRIEFING TO CREW (NOT A SHORT SUMMARY!):\n"
        ...
        "          * 💡 *Additional Online Findings & Extra Details*: YOU ARE NOT LIMITED TO THE CATEGORIES ABOVE! Include ANY and ALL other information, background facts, pricing/rates, surrounding attractions, security/catering policies, contact names, past event info, or any extra details discovered on the internet so the crew has 100% of the available information."
    )
